...
df = pd.DataFrame(columns=['ticker', 'sentiment score'])
...
def get_sentiment_score(ticker):
    # load sentiment data
    sentiment = finnhub_client.stock_social_sentiment(ticker, dr.unix_to_date(dr.from_date_unix), dr.unix_to_date(dr.to_date_unix))
...
def write_sentiment_data(sentiment):
...
def save_all_scores():
    if(config.all_tickers):
        ...
        for i in range(len(ticker_list)):
            # print progress
            seconds_remaining = (len(ticker_list)-i)*api.RATE_LIMIT/api.CALLS
            ...
            # add the sentiment score to the data frame
            api.check_limit()
            sentiment_score = get_sentiment_score(ticker_list[i])
            df.loc[i] = [ticker_list[i], sentiment_score]
    else:
        # save all scores to the data frame
        for i in range(len(config.ticker_list)):
            # print progress
            seconds_remaining = (len(config.ticker_list)-i)*api.RATE_LIMIT/api.CALLS
            minutes_and_seconds_remaining = str(int(seconds_remaining/60)) + ' minutes and ' + \
                str(int(seconds_remaining%60)) + ' seconds'
            ...
            df.loc[i] = [config.ticker_list[i], sentiment_score]
