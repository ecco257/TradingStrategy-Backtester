...
def add_volume_data(price_data):
    # add the volume data to the price data
    price_data['volume'] = price_data['v']
    ...
    for i in range(0, len(price_data['t'])):
        if i < 14:
            ...
            ma.append(sum(price_data['volume'][i-14:i])/14)
    price_data['sma_v'] = ma
    return price_data
...
def get_buy_and_sell_points_stoch(price_data, buy_thres, sell_thres):
    ...
    for i in range(1, len(price_data['t'])):
        ...
        if price_data['slowk'][i-1] < buy_thres and price_data['slowk'][i] > buy_thres and price_data['slowk'][i] > \
            price_data['slowd'][i] and price_data['slowk'][i-1] < price_data['slowd'][i-1] and price_data['emv'][i] - \
            price_data['emv'][i-1] > 30:
            df.loc[len(df)] = [dr.unix_to_date(price_data['t'][i]),price_data['c'][i], 'buy']

        elif price_data['slowk'][i-1] > sell_thres and price_data['slowk'][i] < sell_thres and price_data['slowk'][i] <\
            price_data['slowd'][i] and price_data['slowk'][i-1] > price_data['slowd'][i-1] and price_data['emv'][i-1] -\
            price_data['emv'][i] > 30:
...
# get the optimal buy and sell points based on the stochastic oscillator
def get_optimal_buy_and_sell_points_stoch(price_data, buy_thres, sell_thres):
    # initialize the data frame that will be used to store the time stamps to buy or sell the ticker and whether it is a 
    # buy or sell
    df = pd.DataFrame(columns=['time', 'price', 'buy/sell/hold'])
    # initialize everything to hold
...
def plot_volume(indicator_data):
    ...
    fig.add_trace(go.Scatter(x=[dr.unix_to_date(t) for t in indicator_data['t']], y=indicator_data['sma_v'], mode='lines', name='sma volume'))
